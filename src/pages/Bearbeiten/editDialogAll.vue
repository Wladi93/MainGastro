    ...
    <q-card style="min-width: 350px; width: 1000px">
      ...
      <q-card-section class="q-pt-none">
        <q-form @submit="updateItem" class="q-gutter-md">
          ...
          <div class="q-gutter-sm">
            ...
            <div v-if="uploadStatus !== 'idle'" class="q-mt-sm">
              <q-linear-progress
                ...
                class="q-mt-xs"
              />
              <div v-if="uploadStatus === 'success'" class="text-green text-caption">
                <q-icon name="check_circle" /> Neues Bild erfolgreich hochgeladen
              </div>
              <div v-if="uploadStatus === 'error'" class="text-red text-caption">
            ...
            <div v-if="editImagePreview" class="q-mt-sm">
              ...
              />
              <div v-if="uploadStatus === 'success'" class="text-green text-caption q-mt-xs">
            ...
            <div v-else-if="editItem.img && !editImagePreview" class="q-mt-sm">
              ...
              <div style="display: flex; justify-content: center">
                <q-card style="display: flex; justify-content: center; height: 101px; width: 101px">
                  <q-img
          ...
          <div v-if="hasSizes === true" class="text-subtitle2 q-mt-md">Preise:</div>
          ...
          <q-input
            ...
            step="0.01"
            min="0.00"
            :rules="[(val) => parseFloat(val) > 0 || 'Preis für Klein ist erforderlich']"
          ...
          <q-input
            ...
            step="0.01"
            min="0"
            :rules="[(val) => parseFloat(val) > 0 || 'Preis für Mittel ist erforderlich']"
          ...
          <q-input
            ...
            step="0.01"
            min="0"
            :rules="[(val) => parseFloat(val) > 0 || 'Preis für Groß ist erforderlich']"
      ...
      <q-card-actions align="right" class="text-primary">
        <q-btn flat label="Abbrechen" @click="closeEditDialog" color="negative" />
        <q-btn
          flat
          label="Speichern"
          @click="updateItem"
...
const handleFocus = (size: 'klein' | 'mittel' | 'groß') => {
  const currentRef = size === 'klein' ? kleinPrice : size === 'mittel' ? mittelPrice : großPrice
...
const uploadImage = async (file: File): Promise<string> => {
  const uploadEndpoint = props.uploadEndpoint || 'http://localhost:5008/api/uploads/images'
...
const updateItem = async () => {
  ...
  if (props.hasSizes) {
    const kleinValue = parseFloat((kleinPrice.value || '').toString().replace(',', '.')) || 0
    const mittelValue = parseFloat((mittelPrice.value || '').toString().replace(',', '.')) || 0
    const großValue = parseFloat((großPrice.value || '').toString().replace(',', '.')) || 0
    ...
    const singleValue = parseFloat((singlePrice.value || '').toString().replace(',', '.')) || 0
    editItem.price = singleValue

    if (!editItem.name || !editItem.img || !editItem.description || singleValue <= 0) {
      $q.notify({
        type: 'negative',
        message: 'Bitte füllen Sie alle Felder korrekt aus',
      })
  ...
  try {
    ...
    const response = await fetch(`${BASE_URL}api/categoryitems/${editItem.id}`, {
      method: 'PUT',
      headers: {
    ...
    if (
      ...
      try {
        ...
        console.warn('JSON Parse Error (möglicherweise leere Response):', jsonError)
      }
    }
    if (
      newUploadedImageUrl.value &&
