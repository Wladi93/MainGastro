  ...
  <q-dialog v-model="createDialog" persistent>
    <q-card style="min-width: 350px; width: 1000px">
      ...
      <q-card-actions align="right" class="text-primary">
        <q-btn flat label="Abbrechen" @click="closeCreateDialog" />
        <q-btn
          flat
...
const createCategoryItemSizes = async (categoryItemId: number, sizes: SizeData[]) => {
  try {
    for (const size of sizes) {
      const response = await fetch('http://localhost:5008/api/categoryItemSizes', {
        method: 'POST',
        headers: {
...
const createItem = async () => {
  ...
  if (hasSizesLocal.value) {
    const kleinValue = parseFloat(kleinPrice.value.toString().replace(',', '.'))
    const mittelValue = parseFloat(mittelPrice.value.toString().replace(',', '.'))
    ...
    const singleValue = parseFloat(singlePrice.value.toString().replace(',', '.'))
  ...
  try {
    const categoryItemResponse = await fetch('http://localhost:5008/api/categoryItems', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
